...
- **Запуск:** локально (Uvicorn + Vite dev server) или через Docker (frontend + backend)
...
- сбор обратной связи (фидбэка);
- аналитические метрики и прогнозы;
- экспорт данных для отчётности.

Архитектурно система разделена на:
- **фронтенд** (React + TypeScript + Ant Design), который отвечает за UI и взаимодействие с API;
- **бэкенд** (FastAPI), который реализует REST API, авторизацию, бизнес-логику и хранение данных в PostgreSQL.
...
🧩 Примечание (демо-режим):
- через регистрацию может создаваться первая учётная запись администратора, чтобы получить полный доступ к системе.
...
✅ Итог: нельзя «повысить права» на фронтенде — даже если элементы UI показаны вручную, **API заблокирует запрещённые действия**.
...
⚙️ Конфигурация API:
- базовый URL берётся из переменной окружения (например, `VITE_API_URL`), со значением по умолчанию вроде `http://localhost:8000/api/v1`.
...
- **Не удаётся войти с правильными данными**
  - Проверь URL запросов (`/api/v1/...`), доступность бэкенда, Network-логи браузера.
  - `401` — неверная пара email/пароль  
  - `422` — неверный формат данных (JSON vs form-data и т.п.)  
  - `500` — ошибка сервера (смотри логи бэкенда)
...
🧷 UI-нюанс:
- может отображаться метка вроде «≤ 14 дней до конца» для курсов, у которых скоро дата окончания (логика рассчитывается на фронтенде по `end_date`).
...
- важно избегать N+1 запросов при загрузке связанных сущностей;
- корректный подход — использовать eager loading (например, `selectinload`).
...
Редактирование выполняется через ту же форму, что и создание, но с предзаполненными значениями. После сохранения (обычно `PATCH /api/v1/courses/{id}`) запись в таблице обновляется, пользователю показывается уведомление об успехе.
...
1. **Фильтр курса**  
   ...
   Например, преподаватель может посмотреть статистику только по своему курсу или сразу по всем своим курсам.
...
* Настроить подключение к БД (например, через переменную окружения `DATABASE_URL`).
  В dev можно использовать локальный PostgreSQL или временно SQLite для быстрого теста.
...
* (Опционально) заполнить демо-данными через `seed.py` (если он есть):
...
1. Собрать образ бэкенда (из корня проекта):

```bash
docker build -t itam-backend ./backend
```

2. Запустить контейнер бэкенда:

```bash
docker run --rm -p 8000:8000 itam-backend
```

3. (Если есть `frontend/Dockerfile`) собрать образ фронтенда:

```bash
docker build -t itam-frontend ./frontend
```

4. Запустить контейнер фронтенда:

```bash
docker run --rm -p 5173:5173 itam-frontend
```

В зависимости от Dockerfile фронтенд-контейнер может поднимать dev-сервер (можно сразу открыть [http://localhost:5173](http://localhost:5173)) или отдавать собранный бандл.

---

## Использование готовых образов (GHCR)

Если настроена публикация образов в GitHub Container Registry, можно скачать и запустить их напрямую (имена искать в workflow). Пример логики:

```bash
docker pull ghcr.io/yourname/itam-backend:latest
docker run -p 8000:8000 ghcr.io/yourname/itam-backend:latest

docker pull ghcr.io/yourname/itam-frontend:latest
docker run -p 5173:5173 ghcr.io/yourname/itam-frontend:latest
```

---

## Важно про адрес API при Docker-запуске

Если фронтенд и бэкенд крутятся в Docker на одной машине, фронтенду нужно корректно “видеть” сервер:

* можно использовать `VITE_API_URL=http://host.docker.internal:8000/api/v1` (если фронт локально, бэк в Docker),
* либо при Docker Compose настроить сеть сервисов и обращаться к бэкенду по имени сервиса.
